...
// TinyModInjector.pch will be the pre-compiled header
...
#if (defined(tiny_wtrace) && defined(_WINDOWS))
// https://yal.cc/printf-without-standard-library/
void wtrace(const wchar_t* pszFormat, ...) {
	wchar_t buf[1025];
	va_list argList;
	va_start(argList, pszFormat);
	...
	buf[len] = '\n';
	buf[++len] = 0;
	WriteFile(GetStdHandle(STD_OUTPUT_HANDLE), buf, sizeof(wchar_t) * len, &done, NULL);
}
#endif
